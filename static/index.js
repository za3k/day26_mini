...
function main() {
    ...
    function refreshSave() {
        $(".btn").each((i, button) => {
            const slot = Number(button.getAttribute("slot"));
            $(button).toggleClass("full", !!load(slot));
        });
        $(".preview").each((i, preview) => {
            const slot = Number(preview.getAttribute("slot"));
            const previewState = load(slot);
            if (previewState) {
    ...
    refreshSave();
    $(".save").each((i, e) => {
        const slot = Number(e.getAttribute("slot"));
        $(e).on("click", (ev) => {
            save(slot, state);
            refreshSave();
        });
    ...
    $(".enable-autoreload").on("click", (ev) => {
        const slot = Number(ev.target.getAttribute("slot"));
        autoreloadSlot = slot;
        autoreload = true;
        if (Game.lost(state)) restart();
        $(".enable-autoreload").hide();
        $(".disable-autoreload").show();
    });
    $(".disable-autoreload").on("click", () => {
        autoreload = false;
        $(".enable-autoreload").show();
        $(".disable-autoreload").hide();
    });
    $(".enable-autorestart").on("click", () => {
        autorestart = true;
        $(".enable-autorestart").hide();
        $(".disable-autorestart").show();
    });
    $(".disable-autorestart").on("click", () => {
        autorestart = false;
        $(".enable-autorestart").show();
        $(".disable-autorestart").hide();
    });
    $(".tick").on("click", step);
